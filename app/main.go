...
func main() {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		// Add a label to the counter to differentiate between the two paths
		if urlPath == "/hello" {
			labels = append(labels, attribute.String("path", "/hello"))
		}
	...
	mux.Handle("/world", otelhttp.NewHandler(handler, "/world"))
	server := &http.Server{
		Addr:    ":9000",
		Handler: mux,
	}
